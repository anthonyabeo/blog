...
What I have learned, though, is that those defaults quietly shape everything that follows. A compiler spends most of its life walking and rewriting structured data: tokens, syntax nodes, symbol tables, types, intermediate values, instructions, and diagnostics. When those representations are scattered across the heap, every later pass pays for it in locality, indirection, and complexity. Chandler Carruth makes this case directly in his CppCon 2022 talk, [*Carbon and Data-Oriented Design: Lessons Learned in Compiler Design*](https://www.youtube.com/watch?v=rHIkrotSwcc), where he argues that data layout should be treated as a first-order architectural concern. Andrew Kelley's [*A Practical Guide to Applying Data-Oriented Design*](https://www.youtube.com/watch?v=IroPQ150F6c) makes the same point from the hardware side: if your access patterns fight the machine, the machine wins.
