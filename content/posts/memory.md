...
Asserting both the set and reset lines produces Q=0 and ~Q=0. One must make sure that both R and S are not asserted at the same time because the circuit does not behave deterministic in this state.
...
Clearing the WE line will ensure that the value currently store in the circuit is not modified.
